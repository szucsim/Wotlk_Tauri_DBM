...
function mod:OnCombatStart(delay)
	...
	if time == 0 then
		...
		if DBM:GetRaidRank() == 2 then
			SendChatMessage("{skull}First Algalon pull, 25 seconds to combat. 60 min. until Algalon transmits distress signal.{skull}", "RAID")
		end
	elseif time ~= 0 then
		timerCombatStart:Start(8-delay)
		self:ScheduleMethod(8-delay, "startTimers")	-- 8 seconds roleplaying
		if DBM:GetRaidRank() == 2 then
			SendChatMessage("{skull}Algalon pulled, 8 seconds to combat. " .. time .. " min. until Algalon transmits distress signal.{skull}", "RAID")
		end
	end
end
